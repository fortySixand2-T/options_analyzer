...
def plot_option_model_errors(strikes, calc_prices, market_mids, expiry_label="", save_path=None):
    ...
    Creates two side-by-side plots with independent scales:
    - Left: Absolute error in dollars
    ...
    # Left plot: Absolute error in dollars
    ax1.plot(strikes, errors, marker='o', color='blue', linewidth=2, markersize=8)
    ax1.axhline(0, color='gray', linestyle='--', linewidth=1)
    ax1.set_xlabel('Strike Price', fontsize=11)
    ax1.set_ylabel('Absolute Error ($)', fontsize=11)
    ax1.set_title(f'Absolute Error - Expiry {expiry_label}', fontsize=12, fontweight='bold')
    ax1.grid(True, alpha=0.3)

    # Right plot: Percentage error
    ax2.plot(strikes, pct_errors, marker='x', color='red', linewidth=2, markersize=8)
    ax2.axhline(0, color='gray', linestyle='--', linewidth=1)
    ax2.set_xlabel('Strike Price', fontsize=11)
    ax2.set_ylabel('Percent Error (%)', fontsize=11)
    ax2.set_title(f'Percentage Error - Expiry {expiry_label}', fontsize=12, fontweight='bold')
...
def test_atm_options_today(ticker="AAPL", min_days_to_expiry=30, max_options=10, plot_mode="show", plot_dir="./plots"):
    """Analyze true ATM options (±5% from stock price) from today's available expirations.

    Uses market implied volatility for theoretical pricing and plots model errors.
...
if __name__ == "__main__":
    test_atm_options_today("TSLA", plot_mode="save", plot_dir="./analysis_results")
    test_atm_options_today("NVDA", plot_mode="save", plot_dir="./analysis_results")
    test_atm_options_today("SPY", plot_mode="save", plot_dir="./analysis_results")
